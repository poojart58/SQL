...
select company_id,employee_id,employee_name,salary,cast(Tax_Calculation*salary as integer) as salary
from(
select case when Company_earning<1000 then 1
when (Company_earning>=1000 and Company_earning<=10000) then 0.76
when Company_earning>10000 then 0.51 end as Tax_Calculation,
company_id,employee_id,employee_name,salary
from (
select max(Salary) over(partition by company_id ) as Company_earning,
company_id,employee_id,employee_name,salary
from Salaries
    )
    );
